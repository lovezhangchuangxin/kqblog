  ...
  return (
    <div className="p-5 rounded-xl border border-border bg-background hover:border-primary/25 transition-all duration-300">
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-sm font-medium text-muted-foreground">访客统计</span>
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div>
          <div
            className={`text-3xl font-bold text-primary font-mono transition-all duration-500 ${
              animated && mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'
            }`}
          >
            {mounted ? totalVisits.toLocaleString() : '---'}
          </div>
          <div className="text-xs text-muted-foreground mt-1">累计访问</div>
        </div>
        <div>
          <div
            className={`text-3xl font-bold text-accent font-mono transition-all duration-500 delay-100 ${
              animated && mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'
            }`}
          >
            {mounted ? todayVisits : '--'}
          </div>
          <div className="text-xs text-muted-foreground mt-1">今日访问</div>
        </div>
      </div>
    </div>
  );
